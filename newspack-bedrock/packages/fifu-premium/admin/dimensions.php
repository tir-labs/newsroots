...
function fifu_image_downsize($out, $att_id, $size) {
    ...
    if (fifu_is_off('fifu_photon') || !get_option('fifu_main_domain')) {
        return $out;
    }
    ...
    if (fifu_ends_with($original_image_url, '.svg') && fifu_is_off('fifu_otfcdn'))
        return $out;
    ...
    if ($size === 'full') {
        ...
        if (!empty($metadata['width']) && !empty($metadata['height'])) {
            ...
            if (fifu_is_on('fifu_otfcdn')) {
                $otf_url = $resized_url = fifu_otf_get_image_url($att_id, $image_url, null);
                $dimensions = fifu_otf_get_original_image_dimensions($otf_url);

                $metadata = wp_get_attachment_metadata($att_id);
                if (!is_array($metadata)) {
                    $metadata = [];
                }
                $metadata['width'] = $large_width = $dimensions[0] ?? 0;
                $metadata['height'] = $large_height = $dimensions[1] ?? 0;
...
function fifu_resize_with_photon($url, $width, $height, $crop, $att_id, $size) {
    ...
    if (fifu_is_on('fifu_otfcdn')) {
        if (!$width && !$height) {
            $url = fifu_otf_get_image_url($att_id, $url, null);
            return $url;
        }
        ...
        $url = fifu_otf_get_image_url($att_id, $url, "?w={$w}&h={$h}&c={$c}");
        return $url;
    } else {
        if (!$width && !$height) {
