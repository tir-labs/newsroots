...
function payment_info() {
    ...
    jQuery.ajax({
        method: "POST",
        url: restUrl + 'fifu-premium/v2/payment_info/',
        async: true,
        beforeSend: function (xhr) {
            xhr.setRequestHeader('X-WP-Nonce', fifuScriptVars.nonce);
        },
    ...
    jQuery("#su-dialog-cancel").dialog({
        ...
        buttons: {
            [fifuScriptCloudVars.dialogYes]: function () {
                ...
                jQuery.ajax({
                    method: "POST",
                    url: restUrl + 'fifu-premium/v2/cancel/',
                    async: true,
                    beforeSend: function (xhr) {
                        xhr.setRequestHeader('X-WP-Nonce', fifuScriptVars.nonce);
                    },
...
function listAllMediaLibrary(page, type, keyword) {
    ...
    jQuery.ajax({
        method: "POST",
        url: restUrl + 'fifu-premium/v2/list_all_media_library/',
        data: {
            "page": page,
            "type": type,
            "keyword": keyword,
...
async function convertFifu(table) {
    let selected = table.rows({selected: true});
    let count = selected.count();
    ...
    for (let i = 0; i < count; i++) {
        ...
        arr.push([
            ...
            data[5], // url
            data[6], // thumbnail_id
            data[7], // product_image_gallery
            data[8]  // category
        ]);
        if (i + 1 == count || (i > 0 && i % MAX_ROWS_BY_REQUEST == 0)) {
            jQuery.ajax({
                method: "POST",
                url: restUrl + 'fifu-premium/v2/convert_to_fifu/',
                data: {
                    "selected": arr,
                },
                async: true,
...
function listDailyCount() {
    ...
    jQuery.ajax({
        method: "POST",
        url: restUrl + 'fifu-premium/v2/list_daily_count/',
        async: true,
        beforeSend: function (xhr) {
            xhr.setRequestHeader('X-WP-Nonce', fifuScriptVars.nonce);
        },
...
function set_upload_auto() {
    ...
    jQuery.ajax({
        method: "POST",
        url: restUrl + 'fifu-premium/v2/cloud_upload_auto/',
        data: {
            "toggle": toggle,
        },
        async: true,
...
function set_delete_auto() {
    ...
    jQuery.ajax({
        method: "POST",
        url: restUrl + 'fifu-premium/v2/cloud_delete_auto/',
        data: {
            "toggle": toggle,
        },
        async: true,
...
function set_hotlink() {
    ...
    jQuery.ajax({
        method: "POST",
        url: restUrl + 'fifu-premium/v2/cloud_hotlink/',
        data: {
            "toggle": toggle,
        },
        async: true,
...
function fifu_api_cron(toggleId) {
    ...
    toggle = jQuery(`#${toggleId}`).attr('class');
    switch (toggle) {
        case "toggleon":
            option = "cron-add";
            break;
