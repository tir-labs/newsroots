...
jQuery(function () {
    ...
    jQuery("form[id^=fifu_form]").each(function (i, el) {
        if (el && el.id === 'fifu_form_auto_share_test') {
            return true;
        }
    ...
    // show settings
    window.scrollTo(0, 0);
    if (fifuScriptVars.lock || fifuScriptVars.emptyKey) {
        let currentUrl = new URL(window.location.href);
        if (currentUrl.searchParams.get('page') !== 'fifu-license-key') {
            currentUrl.searchParams.set('page', 'fifu-license-key');
...
function isClickable(id) {
    return id.match("fifu_form_slider_speed") || id.match("fifu_form_slider_pause") || id.match("fifu_form_auto_set_dimensions") || id.match("fifu_form_spinner");
}
...
function refresh() {
    setInterval(function () {
        jQuery('div[id^=fifu_ajax]').load('admin.php?page=fifu-premium div[id^=fifu_ajax]');
    }, 1000)
}
...
function fifu_default_js() {
    ...
    jQuery.ajax({
        method: "POST",
        url: restUrl + 'fifu-premium/v2/' + option + '/',
        async: true,
...
function fifu_sizes_js() {
    ...
    jQuery.ajax({
        method: "POST",
        url: restUrl + 'fifu-premium/v2/load-sizes-api/',
        async: true,
        beforeSend: function (xhr) {
            xhr.setRequestHeader('X-WP-Nonce', fifuScriptVars.nonce);
        },
...
function fifu_reset_sizes() {
    ...
    jQuery.ajax({
        method: "POST",
        url: restUrl + 'fifu-premium/v2/reset-sizes-api/',
        async: true,
        beforeSend: function (xhr) {
            xhr.setRequestHeader('X-WP-Nonce', fifuScriptVars.nonce);
        },
...
function fifu_save_sizes() {
    ...
    jQuery.ajax({
        method: "POST",
        url: restUrl + 'fifu-premium/v2/save-sizes-api/',
...
function fifu_run_clean_js() {
    ...
    jQuery.ajax({
        method: "POST",
        url: restUrl + 'fifu-premium/v2/data_clean_api/',
        async: true,
        beforeSend: function (xhr) {
            xhr.setRequestHeader('X-WP-Nonce', fifuScriptVars.nonce);
        },
...
function fifu_update_all_js() {
    ...
    jQuery.ajax({
        method: "POST",
        url: restUrl + 'fifu-premium/v2/update_all_api/',
        async: true,
        beforeSend: function (xhr) {
            xhr.setRequestHeader('X-WP-Nonce', fifuScriptVars.nonce);
        },
        ...
        complete: function () {
            setTimeout(function () {
                ...
                contentIntervalId = setInterval(updateContentCounter.bind(null, true), 3000);
            }, 1000);
        },
        timeout: 0
    });
...
function fifu_api_cron(toggleId) {
    jQuery('#tabs-top').block({message: fifuScriptVars.wait, css: {backgroundColor: 'none', border: 'none', color: 'white'}});

    let toggle = jQuery(`#${toggleId}`).attr('class');
    switch (toggle) {
        case "toggleon":
            option = "cron-add";
            break;
...
function updateMetadataCounter(transient) {
    jQuery.ajax({
        url: `${restUrl}fifu-premium/v2/metadata_counter_api/`,
        data: {
            "transient": transient,
        },
        method: 'POST',
...
function updateContentCounter(transient) {
    jQuery.ajax({
        url: `${restUrl}fifu-premium/v2/content_counter_api/`,
        data: {
            "transient": transient,
        },
        method: 'POST',
        ...
        success: function (response) {
            jQuery('#image_content_counter').text(response);

            let contentCounterValue = parseInt(jQuery('#image_content_counter').text().trim());
            if ((contentCounterValue === 0 && jQuery('#fifu_toggle_update_all').hasClass('toggleoff'))) {
                if (typeof contentIntervalId !== 'undefined')
...
function fifu_check_for_updates() {
    const licenseKey = jQuery('#fifu_input_key').val()?.trim();
    const $btn = jQuery('#fifu_form_key input[type=submit]');
    ...
    jQuery.ajax({
        method: 'POST',
        url: restUrl + 'fifu-premium/v2/check_for_updates_api/',
        beforeSend: function (xhr) {
            xhr.setRequestHeader('X-WP-Nonce', fifuScriptVars.nonce);
        },
        data: {
            key: licenseKey,
            network: fifuScriptVars?.networkAdmin ? 1 : 0,
        },
        timeout: 60000,
        success: function (data) {
            if (data && data.status) {
                ...
                setTimeout(function () {
                    ...
                    currentUrl.searchParams.set('page', 'fifu-premium');
                    window.location.href = currentUrl.toString();
                }, 1000);
            } else {
                jQuery('#tabs-top').unblock();
...
function fifu_otfcdn_js() {
    jQuery('#tabs-top').block({message: fifuScriptVars.wait1, css: {backgroundColor: 'none', border: 'none', color: 'white'}});

    let toggle = jQuery("#fifu_toggle_otfcdn").attr('class');

    jQuery.ajax({
        method: "POST",
        url: restUrl + 'fifu-premium/v2/otfcdn-api/',
        async: true,
        beforeSend: function (xhr) {
            xhr.setRequestHeader('X-WP-Nonce', fifuScriptVars.nonce);
        },
