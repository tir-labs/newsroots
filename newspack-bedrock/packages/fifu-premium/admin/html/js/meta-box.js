...
function runPreview($url) {
    ...
    if ($url) {
        if (jQuery('#fifu_screenshot').is(":checked")) {
            $url = 'https://screenshot.fifu.app/' + $url;
...
jQuery(document).ready(function () {
    ...
    // Store initial visibility state of upload button BEFORE any UI mutations
    jQuery("#fifu_upload").data('initially-visible',
            jQuery("#fifu_upload").is(':visible'));
    ...
    jQuery("#fifu_button").on('click', function () {
        ...
        if (!$url.startsWith("http") && !$url.startsWith("//")) {
            ...
            } else if (fifu_get_post_title()) {
                fifu_start_lightbox($url, false, null, null, 'meta-box');
            }
        }
    });
    ...
    function updateWpFeaturedImagePanel() {
        var url = jQuery('#fifu_input_url').val();
        var videoUrl = jQuery('#fifu_video_input_url').val();
        var $postImageDiv = jQuery('#postimagediv');
        var $toggleBtn = $postImageDiv.find('.handlediv');

        if ((url && url.trim()) || (videoUrl && videoUrl.trim())) {
            fifu_hide_wp_featured_image_section();
    ...
    jQuery('#fifu_input_url, #fifu_video_input_url').on('input keyup paste', updateWpFeaturedImagePanel);

    // Fallback: poll for value changes (covers autocomplete by mouse)
    let lastFifuUrl = jQuery('#fifu_input_url').val();
    let lastFifuVideoUrl = jQuery('#fifu_video_input_url').val();
    setInterval(function () {
        let current = jQuery('#fifu_input_url').val();
        let currentVideo = jQuery('#fifu_video_input_url').val();
        if (current !== lastFifuUrl || currentVideo !== lastFifuVideoUrl) {
            lastFifuUrl = current;
            lastFifuVideoUrl = currentVideo;
            updateWpFeaturedImagePanel();
        }
    }, 300);
    ...
    // Store initial visibility state of upload button
    jQuery("#fifu_upload").data('initially-visible',
            jQuery("#fifu_upload").is(':visible'));
...
function fifu_get_image(url) {
    ...
    image.onload = function () {
        ...
        if (jQuery("#fifu_upload").data('initially-visible') === true) {
...
function fifu_upload_images_api() {
    ...
    setTimeout(function () {
        let url = fifu_convert(jQuery("#fifu_input_url").val());
        let alt = jQuery("#fifu_input_alt").val();
        let urls = '';
        let alts = '';
        if (fifuMetaBoxVars.is_product) {
            var count = 0;
            jQuery('input[id^="fifu_input_url_"]').each(function (index) {
                let url_aux = jQuery(this).attr('value');
                if (url_aux) {
                    urls += count == 0 ? url_aux : '|' + url_aux;
                    let alt_aux = jQuery('#fifu_input_alt_' + index).val();
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                // remove                
                setTimeout(function () {
                    removeImage(true); // called from upload button
                    if (fifuMetaBoxVars.is_product) {
                        var count = 0;
                    ...
                    if (!fifuMetaBoxVars.is_product && !fifuMetaBoxVars.is_classic_editor) {
                        ...
                        setTimeout(function () {
                            ...
                            if (obj.att_ids.length > 1) {
                                let ids_str = '';
                                for (let i = 1; i < obj.att_ids.length; i++)
...
(function ($) {
    function toggleCategoryThumbnail(isUserAction = false) {
        ...
        var videoUrl = ($('#fifu_video_input_url').val() || '').trim();
    ...
    $(document).ready(function () {
        ...
        $('#fifu_input_url, #fifu_video_input_url')
...
(function () {
    try {
        ...
        function fetchAndApplyFifuUrl(postId) {
            ...
            const url = (base.endsWith('/') ? base : base + '/') + 'fifu-premium/v1/url/' + postId + '?_ts=' + Date.now();
            fetch(url, {
                method: 'GET',
                headers: {
