...
function fifu_load_textdomain() {
    ...
    $mo_file_path = WP_LANG_DIR . "/plugins/fifu-premium/" . fifu_version_number() . "/fifu-premium-$locale.mo";
    ...
    if (file_exists($mo_file_path)) {
        load_textdomain('fifu-premium', $mo_file_path);
    } else {
...
function fifu_download_translation_files($locale) {
    $remote_base_url = 'https://storage.googleapis.com/fifu-translations/fifu-premium/' . fifu_version_number();

    // Convert locale to lowercase and replace "_" with "-"
    $remote_locale = strtolower(str_replace('_', '-', $locale));
    $mo_url = "$remote_base_url/fifu-premium-$remote_locale.mo";

    // Target directory
    $target_dir = WP_LANG_DIR . '/plugins/fifu-premium/' . fifu_version_number() . '/';
    if (!is_dir($target_dir)) {
        wp_mkdir_p($target_dir);
    }

    // Download .mo file
    $mo_target = $target_dir . "fifu-premium-$locale.mo";
