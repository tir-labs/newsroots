...
if (!class_exists('RapidAddonFifu')) {

    class RapidAddonFifu {
    ...
    <h3 class="hndle" style="margin-top:0;"><span>' . $acf_group['title'] . '</span></h3>
        ...
        function import($importData, $parsedData) {
            ...
            $import_options = (isset($importData['import']) && isset($importData['import']['options']) && isset($importData['import']['options'][$this->slug])) 
                ? $importData['import']['options'][$this->slug] 
                : array();
        ...
        function wpai_api_metabox($post_type, $current_values) {
            ...
            foreach ($this->fields as $field_slug => $field_params) {
                ...
                if (!$this->fifu_wai_render($post_type, $field_slug))
                    continue;
        ...
        function fifu_wai_render($post_type, $field) {
            if (in_array($post_type, ['product'])) {
                if (in_array($field, ['fifu_list_url', 'fifu_list_alt']))
                    return true;

                if (in_array($field, ['fifu_list_video_url']) && fifu_is_on('fifu_video'))
                    return true;
            }
            else {
                if (in_array($field, ['fifu_image_url', 'fifu_image_alt']))
                    return true;

                if (in_array($field, ['fifu_video']) && fifu_is_on('fifu_video'))
                    return true;
            }
            ...
            if (in_array($field, ['fifu_slider_list_url']) && fifu_is_on('fifu_slider'))
                return true;

            if (in_array($field, ['fifu_delimiter']) && (in_array($post_type, ['product']) || fifu_is_on('fifu_slider')))
                return true;

            if (in_array($field, ['fifu_isbn']) && fifu_is_on('fifu_isbn'))
                return true;

            if (in_array($field, ['fifu_asin']) && fifu_is_on('fifu_asin'))
                return true;

            if (in_array($field, ['fifu_finder_url']) && fifu_is_on('fifu_finder'))
                return true;

            if (in_array($field, ['fifu_redirection_url']) && fifu_is_on('fifu_redirection'))
                return true;
        }
