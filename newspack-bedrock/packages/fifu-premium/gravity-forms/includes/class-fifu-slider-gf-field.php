...
class FIFU_GF_Slider_Field extends GF_Field {

    /**
     * @var string $type The field type.
     */
    public $type = 'fifu_slider';
    ...
    public function get_form_editor_field_title() {
        ...
        return $strings['field']['slider']() . ' (FIFU)';
    }
    ...
    public function get_field_input($form, $value = '', $entry = null) {
        ...
        $placeholder_attribute = 'placeholder="' . $strings['placeholder']['image']() . '&#10;' . $strings['placeholder']['image']() . '&#10;' . $strings['placeholder']['image']() . '"';
        $required_attribute = $this->isRequired ? 'aria-required="true"' : '';
        $invalid_attribute = $this->failed_validation ? 'aria-invalid="true"' : 'aria-invalid="false"';
        $disabled_text = $is_form_editor ? 'disabled="disabled"' : '';

        // Prepare the input tag for this field.
        $input = "<textarea rows='5' name='input_{$id}' id='{$field_id}' class='{$class}' {$tabindex} {$logic_event} {$placeholder_attribute} {$required_attribute} {$invalid_attribute} {$disabled_text}>{$value}</textarea>";
    ...
    public function get_value_save_entry($value, $form, $input_name, $lead_id, $lead) {
        ...
        foreach ($urls as $url) {
            $_POST['fifu_slider_input_url_' . $i++] = trim($url);
        }
...
GF_Fields::register(new FIFU_GF_Slider_Field());
