        ...
        if (function_exists('wc_get_product')) {
            ...
            if ($product) {
                ...
                if ($product->is_type('variable')) {
                    $json_ld['offers'] = [];
                    if (function_exists('fifu_get_variation_data')) {
                        $variations = fifu_get_variation_data($post_id);
                        foreach ($variations as $variation_id => $variation) {
                            ...
                            $price = $vp->get_price();
                            if ($price === '' || $price === null)
                                $price = $vp->get_regular_price();
                            if ($price === '' || $price === null)
                                $price = $vp->get_sale_price();
                            if ($price === '' || $price === null)
                                $price = $vp->get_meta('_price', true);
                            if ($price === '' || $price === null)
                                continue;

                            // Use HTTPS enumerations per Google guidelines
                            $availability = $vp->is_in_stock() ? 'https://schema.org/InStock' : 'https://schema.org/OutOfStock';
                            $offer = [
                                '@type' => 'Offer',
                                'url' => $variation['url'],
                                'price' => $price,
                            ...
                            if (method_exists($vp, 'get_date_on_sale_to')) {
                                $v_sale_to = $vp->get_date_on_sale_to();
                                if ($v_sale_to) {
                                    $v_json = $v_sale_to->date_i18n('c');
                                    if ($v_json)
                                        $offer['priceValidUntil'] = $v_json;
                                }
                            }
                    ...
                    // Fallback minimal Offer for variable product when no variation offers could be built
                    if (!$has_valid_offer) {
                        $price = null;
                    ...
                    // If we have multiple offers, prefer AggregateOffer for Google Rich Results
                    if ($has_valid_offer && is_array($json_ld['offers']) && count($json_ld['offers']) > 1) {
                        $prices = [];
                        $any_in_stock = false;
                        foreach ($json_ld['offers'] as $of) {
                            $p = isset($of['price']) ? (float) $of['price'] : null;
                            if ($p !== null)
                                $prices[] = $p;
                            if (($of['availability'] ?? '') === 'https://schema.org/InStock')
                                $any_in_stock = true;
                        }
                        if (!empty($prices)) {
                            $aggregate = [
                            ...
                            $json_ld['offers'] = $aggregate;
                        }
                    }
                } elseif ($product->is_type('grouped')) {
                    // Grouped product: derive AggregateOffer from child products' prices
                    ...
                    foreach ($child_ids as $cid) {
                        ...
                        $price = $cp->get_price();
                        if ($price === '' || $price === null)
                            $price = $cp->get_regular_price();
                        if ($price === '' || $price === null)
                            $price = $cp->get_sale_price();
                        if ($price === '' || $price === null)
                            $price = $cp->get_meta('_price', true);
                        if ($price === '' || $price === null)
                            continue;
                        $prices[] = (float) $price;
                        if ($cp->is_in_stock())
                    ...
                    if (!empty($prices)) {
                        ...
                        if ($any_in_stock) {
                            $json_ld['offers']['availability'] = 'https://schema.org/InStock';
                        }
                        $has_valid_offer = true;
                    }
                } else {
