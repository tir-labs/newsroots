	...
	class Ui {
		...
		public function displayManualCheckResult() {
			...
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->updateChecker->slug) ) {
				...
				if (!fifu_server_check_key()) {
					printf(
						'<div class="notice %s is-dismissible"><p>%s</p>%s</div>',
						'error notice-error',
						apply_filters($this->updateChecker->getUniqueName('manual_check_message'), sprintf('%s: license key is invalid, expired or has been activated on more than 1 domain.', $title), 'error'),
						''
					);
				}
				else {
