...
namespace FluxMedia\FFMpeg\Media;

use FluxMedia\Alchemy\BinaryDriver\Exception\ExecutionFailureException;
use FluxMedia\FFMpeg\Driver\FFMpegDriver;
use FluxMedia\FFMpeg\Exception\RuntimeException;
use FluxMedia\FFMpeg\FFProbe;
use FluxMedia\FFMpeg\Filters\AdvancedMedia\ComplexCompatibleFilter;
use FluxMedia\FFMpeg\Filters\AdvancedMedia\ComplexFilterContainer;
use FluxMedia\FFMpeg\Filters\AdvancedMedia\ComplexFilterInterface;
use FluxMedia\FFMpeg\Filters\AdvancedMedia\ComplexFilters;
use FluxMedia\FFMpeg\Filters\FiltersCollection;
use FluxMedia\FFMpeg\Format\AudioInterface;
use FluxMedia\FFMpeg\Format\FormatInterface;
use FluxMedia\FFMpeg\Format\ProgressableInterface;
use FluxMedia\FFMpeg\Format\ProgressListener\AbstractProgressListener;
use FluxMedia\FFMpeg\Format\VideoInterface;
/**
...
class AdvancedMedia extends AbstractMediaType
{
     ...
     */
    public function map(array $outs, FormatInterface $format, $outputFilename, $forceDisableAudio = false, $forceDisableVideo = false)
    {
        $commands = array();
     ...
     * @return array
     */
    private function applyFormatParams(FormatInterface $format, $forceDisableAudio = false, $forceDisableVideo = false)
    {
        // Set format params.
        $commands = array();
    ...
    protected function assertFiltersAreCompatibleToCurrentFFMpegVersion()
    {
        ...
        foreach ($this->filters as $filter) {
            if (version_compare($currentVersion, $filter->getMinimalFFMpegVersion(), '<')) {
                $messages[] = $filter->getName() . ' filter is supported starting from ' . $filter->getMinimalFFMpegVersion() . ' ffmpeg version';
            }
        }
        if (!empty($messages)) {
    ...
    protected function buildCommand()
    {
        ...
        return array_merge(array('-y'), $this->buildConfiguredGlobalOptions($globalOptions), $this->getInitialParameters(), $this->buildInputsPart($this->inputs), $this->buildComplexFilterPart($this->filters), $this->mapCommands, $this->getAdditionalParameters());
    }
    /**
